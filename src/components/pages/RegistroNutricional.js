...
function RegistroNutricional() {
  ...
  const handleAlimentoChange = async (index, field, value) => {
    ...
    if (field === 'nome') {
      ...
      const alimentoInfo = alimentosData.find(
        alimento => alimento['Descrição do Alimento'] === selectedFood.value
      );
  ...
  return (
    ...
    <form className="registro-form" onSubmit={handleSubmit}>
      ...
      <div className="input-container">
        ...
        <input type="text" id="refeicao" value={refeicao} onChange={(e) => setRefeicao(e.target.value)} />
      </div>
  ...
  <div className="alimento-container" key={index}>
    ...
    <Select
      options={foodOptions}
      value={alimento.selectedFood}  // Altere esta linha
      ...
      {registroDiario.length > 0 ? (
        ...
        {registroDiario.map((registro, index) => (
          <li key={index}>
            ...
            <p className='dataDiaria' >Data: {registro.data}</p>
            <ul className="alimentos-list">
              {registro.alimentos.map((alimento, index) => (
                <li key={index}>
                  ...
                  <span>Proteínas: {alimento.proteinas.toFixed(2)}</span>
                  <span>Carboidratos: {alimento.carboidratos.toFixed(2)}</span>
                  <span>Gorduras: {alimento.gorduras.toFixed(2)}</span>
                </li>
              ))}
            </ul>
          </li>
